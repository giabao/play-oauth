...
class UUIDOauthTokenFactory[RO <: OauthResourceOwner[C, P], C <: OauthClient, P <: OauthPermission[C]] extends OauthTokenFactory[BasicOauthToken, RO, P, C] {
  def apply(owner: RO, client: C, redirectUri: Option[String], scopes: Option[Seq[String]])(implicit ec: ExecutionContext): Future[BasicOauthToken] = Future.successful(new BasicOauthToken(UUID.randomUUID().toString, "example"))
}
